...
updated_at: '2026-09-10T18:25:08.095Z'
transitions:
  - status: in-progress
    at: '2026-09-10T16:38:19.782Z'
  - status: complete
    at: '2026-09-10T18:25:08.095Z'
depends_on:
  - 003-commerce-upgrade
completed_at: '2026-09-10T18:25:08.095Z'
...
本次仅负责 DIY 的 H5、iOS、后端、平台管理台。工作分支 `codex/diy-studio-20260911`；源任务工作区未被编辑。构建前已在独立副本中合入源任务商城最新提交（后端 71b756a、前端 8f8b05a、H5 f735f31）。发布前又合入已上线的社区版本（前端 58b8b18、H5 27736ca），避免覆盖并行发布。新增文档使用 019，保留源任务的 018 编号。
...
- 当前网页发布：`20260911-diy-render-2e6e734`；DIY 容器：`20260911-diy-studio-cfb9609`，healthy。后端 Git `a00488d`、H5 `2e6e734`、前端/iOS/管理台 `91d396b`，均推送至独立分支 `codex/diy-studio-20260911`。
- 线上浏览器再次验证 390/768/1440、多材料 2D/3D、原图 MIME/哈希及商城/社区入口；首次与连续三次 3D 挂载均保留正确颜色。异步纹理到达后显式刷新材质着色器，修复初次加载白色珠子。
- 45 种材料的价格、库存、规格、上架状态与发布前一致；2 种实拍映射与 7 种木纹映射生效。其他 34 个容器 ID 和镜像未变化。未登录保存返回现有网关业务码 `40101`（HTTP 200），无数据写入。
- DIY 数据库备份：`/opt/askxuan/backups/20260911-diy-studio-cfb9609/diy-before.sql`。同目录保存旧镜像对应的 `compose.rollback.json` 和原静态目录；渲染补丁自身回退目录记录在 `/opt/askxuan/runtime/20260911-diy-render-2e6e734/previous-public`。
- 发布中遇到初始化密码变量过期、MySQL 客户端默认字符集问题，已改为使用运行中服务的数据库身份并强制 UTF-8。导入修复仅精确匹配本次新增素材字段。
- ECS 发布前磁盘已满，仅清理旧版可再生成依赖和超过 24 小时未使用的构建缓存。发布后剩余空间约 265 MiB；需要另行安排容量扩展或历史发布保留策略。
- 完整验收证据在工作区 `artifacts/diy-upgrade-20260911/verification/`；包括本地数据库写入闭环与线上只读/渲染检查。iOS 仍为无签名构建验证，未作真机验收。
