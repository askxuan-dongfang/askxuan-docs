...
**网关模型**：自研 net/http + httputil.ReverseProxy，19 条 C 端业务路由 + 1 条 OpenIM 透传路由 + 21 条管理台路由 = 41 条 Prefix；最长前缀匹配，动态服务发现优先、静态 Target 回退
**接口总数**：214 个（C 端 68 + 管理台 146）

**文档结构**：

- **上篇：客户端视角**——6 个客户端各自调用哪些接口（第一至第五章）
- **下篇：后端视角**——17 个后端服务各自提供哪些接口（第六至第二十二章）
...
> 本篇按 17 个后端服务分章，回答"每个后端服务提供哪些接口"。
