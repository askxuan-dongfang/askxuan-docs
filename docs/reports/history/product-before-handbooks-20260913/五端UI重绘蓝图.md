...
> 归档日期：2026-09-13。来源：docs 基线 `6bc43bc8962368348962651f628c5de2b4f8c5de` 的 `docs/product/五端UI重绘蓝图.md`。
> 范围：完整保留该文件在手册整编前的正文，包括其中不同日期的方案、校正、待办和执行记录；原文中的“当前”“已完成”“待实施”只对应各段原有日期，不因归档而成为最新能力或验收结论。
> 阅读当前内容请从[原路径导航](../../../product/五端UI重绘蓝图.md)和[产品使用手册](../../../guides/产品使用手册.md)进入。历史技术命令、演示配置及阶段判断仅供追溯，不直接作为当前操作指令或对外合作材料。
> 保全方式：正文不删改；仅将 Markdown 相对链接重定位到原文件位置所指目标。原始正文 SHA-256：`1c9830d575780683c0e2d8fe0021c9ef0331a2e00fc93a03cd6fc43dedb2f8af`。

---

# 问玄东方五端 UI 重绘蓝图

> 文档版本：v1.2（2026-09-13 校正；历史蓝图 v1.1 保留）
> 编制日期：2026-09-04
> 文档性质：目标体验与实施指导；当前实现、测试与 ECS 发布状态见[五端视觉回归与业务闭环用例](../../../product/五端视觉回归与业务闭环用例.md)
> 当前覆盖：双角色 H5、两套 iOS、统一运营与寺院管理界面、商城兼容包；历史蓝图覆盖五端 Web/H5。

## 2026-09-13 已实施视觉基线

此节为当前设计约束；下方 2026-09-04 蓝图保留原设计过程和模板命名。与本节冲突的“后台固定浅色、H5 固定深色”“公共组件分别维护”等描述已过时。完整能力与证据见 [产品现状与能力边界](../../../product/产品现状与能力边界.md)、[2026-09-13 产品与文档核验](../../2026-09-13-产品与文档核验.md)。

| 维度 | 当前约束 |
|---|---|
| 范围 | 信众/法师 H5、两套 iOS、统一运营与寺院管理界面；商城包为兼容入口 |
| 浅色 | 奶油白页面 `#F6F3EC`、墨绿主操作 `#284D43`、柔金强调 `#84673D`；保留东方珠作的排版和文化气质 |
| 深色 | 深棕页面 `#1C1210`、朱砂主操作 `#C45A3C`、暖金强调 `#C8A96E`；所有角色均可选择，不绑定终端类型 |
| 品牌图形 | 问玄东方/信众、法师、寺院、商城、平台、东方珠作六变体；Logo 可使用独立光学色值，不把每个图形颜色直接当按钮 token |
| 字体 | 共享标题/正文/数字语义；本地宋体资源用于品牌与标题，正文保持可读的系统无衬线；尊重 iOS 动态字体 |
| 动效 | 快速 120ms、常规 200ms、进入 280ms、退出 160ms；明确导航选中、忙态、弹层与数据更新；遵循系统减少动态效果，不靠全页大位移制造反馈 |
| 组件层 | Web 令牌与管理共享组件已有权威源及同步检查；H5 保持 React 层，iOS 保持 SwiftUI 原生层；不把蓝图中的包名建议写成已发布 npm 包 |

源码：[tokens](https://github.com/askxuan-dongfang/askxuan-frontend/blob/8df155727224fcbdbfe67d3eeb3695003eaf24e1/packages/design-tokens/tokens.json)、[typography](https://github.com/askxuan-dongfang/askxuan-frontend/blob/8df155727224fcbdbfe67d3eeb3695003eaf24e1/packages/design-tokens/typography.css)、[motion](https://github.com/askxuan-dongfang/askxuan-frontend/blob/8df155727224fcbdbfe67d3eeb3695003eaf24e1/packages/design-tokens/motion.css)、[品牌生成器](https://github.com/askxuan-dongfang/askxuan-frontend/blob/8df155727224fcbdbfe67d3eeb3695003eaf24e1/packages/brand/generate.mjs)。

本轮落地的高价值交互包括管理移动侧栏焦点/隐藏语义、固定列实底、状态与弹层反馈；H5 DIY 材料库的分类/五行/搜索、折叠零结果恢复、材料结果定位，订单卡的金额/物流/明细层级；原生共享按钮、导航及加载反馈。页面未改变交易金额权威源、权限或状态机。

H10 DIY 模板同时包含可操作 2D 与真 3D；H5 使用 Three.js，iOS 使用 SceneKit。筛选控件按实际端能力表达：H5 有分类/五行/搜索组合，iOS 有分类/关键词筛选，暂无独立五行控件。H06 DIY 订单模板包括确认收货。订单材料数据缺失时降级为描述，不把设计旧价格展示为成交价。真实支付、真机完整验收不属于从视觉结果可以推导的结论。

### 历史蓝图（2026-09-04）

以下目标、原型矩阵、模板、排期与决策保留作历史参考。原勾选和“当前”均指当时范围；以本节和产品现状页判断现在的实现及验收边界。
...
- 权威业务状态以[状态机](../../../architecture/状态机.md)为准，端能力以[功能对齐矩阵](../../../product/功能对齐矩阵.md)为准。
- 现有原型的页面连接完整性以[交互完整性审计报告](https://github.com/reacher1130/AskXuan-Demo/blob/4a21180d3a2d8bb4b4aaf7ff1c053c1c4da68069/交互完整性审计报告.md)为参考。
...
## 19. 历史决策记录（冲突项以 2026-09-13 基线为准）
...
1. [《五端页面—模板—组件映射表》](../../../product/五端页面-模板-组件映射表.md)：逐页标记模板、优先级、责任人、完成状态和验收基线。
2. [《五端视觉回归与业务闭环用例》](../../../product/五端视觉回归与业务闭环用例.md)：固定设备尺寸、账号边界、数据前置条件、操作步骤和期望结果。
