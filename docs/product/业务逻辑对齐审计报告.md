...
> 结论等级: 六项改进及预约计价/支付/容量/gRPC 已对齐，本地验收通过；生产外部 Provider 待配置
...
本地验收已通过六项新增闭环、既有 MVP 回归、21 个 Go workspace module 测试、两个 iOS workspace 编译、三个 Web 管理台构建、Mock 构建及备用 `mobile-customer` TypeScript 检查。预约专项脚本在真实 MySQL/etcd/RabbitMQ 容器中通过 10/10：权威计价、价格防篡改、请求/支付幂等、容量 10 并发 11 单、取消释放、支付中断重试、RPC/MQ 响应丢失后的定时对账、支付超时释放和寺院 RPC 失败关闭均通过。这里的“一致”不等于外部云能力已生产开通：AI 默认使用明确标识的 `mock` Provider，直播默认 `LIVE_ENABLED=false`；生产发布前仍需配置模型、VOD/直播账号、密钥和回调域名。
...
flowchart LR
	...
	B[booking-service] -->|temple.rpc:9083| TempleRpc[temple-service]
	B -->|master.rpc:9084| MasterRpc[master-service]
	B -->|payment.rpc:9090| PaymentRpc[payment-service]
```
...
sequenceDiagram
	...
	participant TS as temple/master gRPC
    participant T as 寺院管理台
    participant M as 法师端
    participant R as review-service
    participant F as finance-service

	C->>B: GET /bookings/availability
	C->>B: POST /api/v1/bookings(requestId/serviceId/slotCode)
	B->>TS: gRPC校验服务价格、时段和法师归属
	B->>B: pending_payment + 原子容量占位
	B->>P: payment.rpc(mock, booking:id 幂等)
	P-->>B: success
    B->>T: 预约待确认
    T->>B: 确认预约
    M->>B: 开始/完成服务
...
一致性判断：客户端一次点击触发服务端计价、容量占位与本地模拟支付；支付成功后才进入寺院待确认。booking-service 通过 temple/master/payment 三个 gRPC 服务校验和支付，不再运行时跨库读取。动态服务、日期时段、剩余容量、价格快照、支付通知与定时对账已在同一契约中对齐。生产环境检测到 `Provider=mock` 会拒绝启动。
