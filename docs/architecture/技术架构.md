...
2026-09-16 产品基线更新：统一邮箱 / 用户名密码与图片验证码认证、双轨入驻、服务执行与回执。教学示例与当前生产契约分开阅读，最新入口见[产品使用手册](../guides/产品使用手册.md)，验证范围见[本次同步报告](../reports/2026-09-16-产品文档同步.md)。旧示例的手机号登录、默认密码及部署变量不可直接用于生产。

源码与架构核对日期：2026-09-14。本文描述产品 **0.0.1** 的当前工程结构、业务边界和运行方式。产品版本不改变 Go / npm 依赖版本、API 路径中的 `v1`、数据库迁移编号或 iOS 构建号。构建通过、源码发布、服务器运行、真机安装和真实交易分别验收。

产品流程见[产品使用手册](../guides/产品使用手册.md)，端差异与待验收项目见[产品现状与能力边界](../product/产品现状与能力边界.md)，完整 HTTP 契约见 [API Reference](../../API-REFERENCE.md)。

## 1. 客户端与业务角色

信众、法师、寺院、商城和平台是业务角色。商城完整并入总管理台后，保留一个 H5 工程、两个原生 iOS 工程和两个业务管理 Web 构建工程：统一管理台与寺院管理台。
...
| 商城兼容入口 `/shop/*` | 统一管理台产物 `dist/legacy/shop/index.html` | 自包含静态迁移页，随 admin 构建、发布；映射详情路径、查询和 hash，不赋予商城账号平台权限 |

`apps/web-shop-admin` 不再作为工程存在，也不参与 npm 安装、组件同步或独立构建。商城页面、路由和业务依赖均归入 `apps/web-platform-admin`；寺院仍由 `apps/web-temple-admin` 独立构建和运行。
...
Refresh Token 绑定登录身份域与客户端；续期按原身份查询对应账号，不能凭相同数字 ID 从信众身份转换成管理身份。缺少身份信息的旧 Refresh Token 失效后需要重新登录，不能以兼容旧会话为由跨域猜测账号。管理请求需有效 access JWT 与管理身份；平台消息模板、推送日志、推送和公告管理由网关及消息服务共同限制为 `UserType=admin` 且 `platform_super`。`/api/v1/admin/messages/master/*` 使用更具体的法师规则，要求 `admin`、`master` 与有效 `MasterID`，按本人消息归属处理。内部通知继续通过 RabbitMQ；人类 `platform_service` 角色不获得平台消息管理权限。
...
│   ├── apps/web-platform-admin/     # 平台与商城业务；dist/legacy/shop/index.html 兼容旧入口
...
后端 Makefile 提供本地栈的预检、启停、编译和检查入口。基础 Compose 包含 MySQL、Redis、RabbitMQ、MinIO、etcd，完整配置叠加 20 个 Go 服务 / 网关；OpenIM 使用独立脚本和 Compose。具体命令以[后端 README](https://github.com/askxuan-dongfang/askxuan-backend/blob/5c1d7b2fc0998cc6ab89b8adb2634b820fa619cf/README.md)和[Go 后端指南](../guides/Go后端指南.md)为准。
