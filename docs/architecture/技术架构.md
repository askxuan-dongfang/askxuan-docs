...
除上述 5 个正式客户端外，仓库保留备用 `mobile-customer`（React Native/Expo）做契约回归。H5 网页与微信小程序仍是后续规划，当前仓库没有对应工程，不计入已实现客户端。
...
| 备用 mobile-customer | React Native + Expo | Expo | 契约与跨端可行性回归，不属于五个正式客户端 |
...
> 说明：本地基础 Compose 提供 MySQL、Redis、RabbitMQ、MinIO、etcd 5 个中间件容器；叠加 `docker-compose.full.yml` 后启动 20 个服务/网关容器，共 25 个 askXuan 容器。OpenIM 使用独立启动脚本和独立 Compose。

---

## 3. 微服务拆分

后端依据业务域拆分为 **19 个下游微服务 + 1 个统一网关**，全部基于 Go/go-zero 实现，通过 etcd 注册并由 gateway 路由。各服务独立监听，客户端统一访问 `http://localhost:8080`。
...
graph TB
    ...
    subgraph 内容域["② 内容域（5服务）"]
		...
		BOOKING["booking-service<br/>:8085<br/>权威计价/容量/支付/预约"]
    ...
    FILE & MEDIA --> MinIO
	BOOKING -. "temple.rpc :9083" .-> TEMPLE
	BOOKING -. "master.rpc :9084" .-> MASTER
	BOOKING -. "payment.rpc :9090" .-> PAY
...
| ---- | ---- | ---- | ---- | ---------- | ---- |
| 1 | **gateway-service** | 8080 | API 网关：路由 / 限流 / 鉴权前置 | - | ✅ 本地闭环 |
| 2 | **auth-service** | 8081 | SSO 认证中心：JWT 签发 / 续期 / 登出 | `admin_account` | ⚠️ 本地闭环；生产认证待接 |
| 3 | **user-service** | 8082 | 用户注册 / 资料 / 角色绑定 | `user`, `user_profile` | ✅ 本地闭环 |
| 4 | **temple-service** | 8083 / gRPC 9083 | 寺院信息 / 图片 / 管理员 / 服务时段 | `temple`, `temple_image`, `temple_admin` | ✅ 本地闭环 |
| 5 | **master-service** | 8084 / gRPC 9084 | 法师信息 / 资质 / 排班 | `master`, `master_credential`, `master_schedule` | ✅ 本地闭环 |
| 6 | **booking-service** | 8085 | 预约计价 / 容量 / 支付 / 状态流转 | `booking`, `booking_slot_inventory`, `booking_status_log` | ✅ 已实现 |
| 7 | **product-service** | 8086 / gRPC 9086 | 商品 / SKU / 分类；心愿导航及寺院/大师服务聚合 | `product`, `product_sku`, `product_category`, `intent_tag` | ✅ 本地闭环 |
| 8 | **diy-service** | 8088 | DIY 设计 / 材料库 / 设计广场下单 | `diy_design`, `diy_order`, `material`, `material_sku` | ✅ 本地闭环 |
| 9 | **order-service** | 8089 / gRPC 9089 | 商城订单 / 物流 | `shop_order`, `shop_order_item`, `shop_order_logistics` | ✅ 本地闭环 |
| 10 | **payment-service** | 8090 / gRPC 9090 | 支付 / 退款 / 本地 mock Provider | `payment`, `payment_log`, `refund` | ✅ 本地支付闭环 |
| 11 | **logistics-service** | 8095 | 物流追踪 / 运费模板 | `logistics_track`, `freight_template` | ⚠️ 本地模拟 Provider |
| 12 | **review-service** | 8092 | 评价 / 举报 | `review`, `report` | ✅ 本地闭环 |
| 13 | **audit-service** | 8093 | 设计广场 / 评论审核 | `audit_queue`, `audit_log` | ✅ 本地闭环 |
| 14 | **finance-service** | 8091 | 平台总账 / 分账 / 提现 / 抽成 | `finance_transaction`, `finance_ledger_entry`, `settlement`, `withdrawal`, `commission_config` | ⚠️ 本地总账分账闭环；生产代付和部分报表待生产化 |
| 15 | **message-service** | 8094 | 站内消息 / Push / OpenIM webhook | `message`, `message_template` | ✅ 本地闭环 |
| 16 | **marketing-service** | 8096 | 优惠券 / 活动 / Banner | `coupon`, `activity`, `banner` | ✅ 本地闭环 |
| - | **ai-service** | 8098 | 版本化技能、确定性路由、文本/视觉、SSE 阶段、额度/安全/动态成本与隔离 MCP | `ai_skill`, `ai_session`, `ai_message`, `ai_run`, `ai_tool_call`, `ai_usage_*` | 本地 Go/H5/iOS 构建通过；ECS 闭环以发布验收记录为准 |
| - | **file-service** | 8097 | 文件上传 / 鉴权 / 手动备份 | - | ✅ 本地闭环 |
...
> 状态标记：✅ 表示本地实现和闭环测试已通过；⚠️ 表示本地可用但依赖外部生产 Provider、真实经营读模型或生产安全配置。
...
| 方案 | 优点 | 缺点 | 适用场景 |
| ---- | ---- | ---- | -------- |
| 共享库 + 租户 ID 隔离 | 统一管理、运维简单 | 运行时跨域联表会扩大权限和故障范围 | 仅保留兼容初始化数据 |
| **独立数据库** | 领域边界清晰、最小权限、可独立演进 | 需设计服务契约和最终一致性 | ✅ 当前：21 个业务库 |
| 分库分表 | 水平扩展能力强 | 开发复杂度高 | 数据量极大时 |

业务互通通过 REST/gRPC 和 RabbitMQ 完成。预约域运行时只读写 `askxuan_booking`：寺院服务、法师归属和支付结果分别通过 `temple.rpc`、`master.rpc`、`payment.rpc` 获取；历史快照回填仅允许迁移脚本使用高权限账号一次性执行。

### 6.2 核心数据表清单

> 与 `askXuan-backend/db/init.sql`（99 条建表声明 / 21 个业务库）对齐。
...
| 服务域 | `service_type` / `temple_service` / `temple_service_slot` | 服务类型、寺院服务项目、结构化时段模板 |
| 预约与咨询域 | `booking` / `consultation_order` / `booking_slot_inventory` / `booking_status_log` / `booking_review` / `booking_chat_message` | 独立即时咨询、预约计价快照、日期容量、评价及统一聊天历史 |
...
| 财务域 | `finance_transaction` / `finance_ledger_entry` / `settlement` / `withdrawal` / `commission_config` / `finance_log` | 平台总账、分账结算、提现、抽成配置、财务日志 |
...
| 按业务域独立数据库 | 服务间使用 REST/gRPC/MQ，运行时遵守最小权限；跨库只读仅保留在有明确文档的聚合场景 |
