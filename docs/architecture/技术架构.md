...
后端依据业务域拆分为 **16 个核心服务 + 2 个扩展服务（ai / file）**，全部基于 go-zero 实现，通过 etcd 注册并被 go-zero gateway 路由。各服务独立端口监听（8080-8098），对外统一由网关 `http://localhost:8080` 暴露。
...
graph TB
    ...
    C1 & C2 & C3 & C4 & C5 --> GW
    GW --> AUTH & USER
    GW --> TEMPLE & MASTER & BOOKING & REVIEW
    GW --> PROD & DIY & ORDER & PAY
    GW --> FIN & AUDIT & LOGISTICS & MKT
    GW --> MSG & FILE & AI

    AUTH & USER & TEMPLE & MASTER & BOOKING & REVIEW --> MySQL & Redis & MQ
    PROD & DIY & ORDER & PAY --> MySQL & Redis & MQ
    FIN & AUDIT & LOGISTICS & MKT --> MySQL & Redis
    MSG & FILE & AI --> MySQL & Redis
    FILE --> MinIO

    AUTH & USER & TEMPLE & MASTER & BOOKING & REVIEW & PROD & DIY & ORDER & PAY & FIN & AUDIT & LOGISTICS & MKT & MSG & FILE & AI -.-> Etcd

    COMMON -.-> AUTH & USER & TEMPLE & MASTER & BOOKING & REVIEW & PROD & DIY & ORDER & PAY & FIN & AUDIT & LOGISTICS & MKT & MSG & FILE & AI
```
...
| `/api/v1/file/**` | file-service | 需 Token |
...
> 与 `askXuan-backend/db/init.sql`（69 表 / 16 库）对齐。
...
| AI事件 | `ai.events` | fanout | ai-service | ai-service | AI对话异步处理 |
