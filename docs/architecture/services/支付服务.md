...
- booking-service 专用 payment.rpc：本地/测试 mock 自动支付，生产环境禁止 mock
...
| idempotency_key | VARCHAR(96) | 业务支付幂等键，预约使用 booking:预约号 |
...
10. **预约模拟支付**：payment.rpc 以 `booking:<bookingNo>` 幂等；支付单创建、pending/success 日志和置成功在同一事务中提交
