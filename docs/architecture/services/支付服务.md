...
- booking-service 专用 payment.rpc：预约和即时咨询使用各自幂等键在本地/测试 mock 自动支付，生产环境禁止 mock
- 当前仅实现 `mock` Provider；微信/支付宝字段为前向兼容契约，不代表已接入渠道 SDK
...
| idempotency_key | VARCHAR(96) | 业务支付幂等键，预约/咨询分别使用 `booking:` / `consultation:` 前缀 |
| order_type | VARCHAR(16) | booking/consultation/shop_order/diy_order |
| order_no | VARCHAR(32) | 关联订单编号 |
| amount | DECIMAL(10,2) | 支付金额 |
| channel | VARCHAR(16) | 当前可执行值为 mock；wechat/alipay 预留 |
...
3. **渠道选择**：当前只支持独立 mock Provider；非 mock 配置和生产环境均启动失败，直至真实渠道 Provider 完成
...
10. **预约与咨询模拟支付**：payment.rpc 以 `booking:<bookingNo>` 或 `consultation:<orderNo>` 幂等；支付单创建、pending/success 日志和置成功在同一事务中提交
...
| Redis | 本地 | 支付单缓存/幂等控制 |
| etcd | 服务注册 | 服务发现 |
| gRPC | payment.rpc :9090 | booking-service 自动支付与结果查询 |
...
| 版本 | 日期 | 说明 |
|------|------|------|
| v1.3 | 2026-07-18 | 明确仅 mock 已实现，禁止用示例 URL 冒充微信/支付宝生产支付 |
| v1.2 | 2026-07-15 | 增加预约 mock Provider、支付幂等事务和 payment.rpc |
