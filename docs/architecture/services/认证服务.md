...
- **OpenIM 集成**（best-effort）：C 端用户登录时同步注册到 OpenIM 并获取 IMToken，随登录响应返回。OpenIM 不可用时 IMToken 为空字符串，不影响核心业务闭环。OpenIM Server 已部署运行（REST 10002 + WS 10001），iOS 端已集成真实 SDK 消费 IMToken
...
1. **多端登录**：C 端走 `/auth/login`（手机号+验证码），管理台走 `/auth/admin/login`（account+password），均签发 JWT
2. **密码安全**：admin_account 密码使用 bcrypt 存储；MVP 阶段 mock 明文，生产须 bcrypt
3. **角色绑定**：创建账号时绑定 role_id，寺院管理员须填 temple_id，法师账号须填 master_id
4. **数据隔离**：temple_admin 仅本寺院数据，master 仅本人数据，platform_super 全平台
5. **登出**：将 Access Token 加入 Redis 黑名单至过期
6. **账号启停**：disabled 状态账号登录直接拒绝（ErrUserDisabled）
7. **OpenIM best-effort 集成**：C 端用户登录成功后，若 `IMClient` 非空则调用 `RegisterUser` + `GetUserToken` 获取 IMToken 并写入 `LoginResp.IMToken`；任何 OpenIM 调用失败均忽略，IMToken 保持空字符串，不影响登录主流程。OpenIM Server 已部署运行（REST 10002 + WS 10001），iOS 端已集成真实 SDK 消费 IMToken
...
| 依赖类型 | 依赖 | 说明 |
|---------|------|------|
| Redis | Token 黑名单 / 验证码 | 登出失效、验证码临时存储 |
