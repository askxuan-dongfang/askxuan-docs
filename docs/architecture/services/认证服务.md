...
2026-09-16 更新。认证、入驻接口增量见 [API Reference](../../../API-REFERENCE.md)；客户端 UI 和操作见[产品使用手册](../../guides/产品使用手册.md)。SMTP 凭据只在服务端受限配置中保存，不写入本手册。
...
auth-service 负责信众、法师、寺院、商城和平台五类角色的认证与权限管理；商城与平台共用总管理台，寺院仍保留独立入口：

- C 端 / 法师端 / 管理台多端登录（用户名 / 邮箱 + 密码 + 图片验证码）
...
| password | VARCHAR(128) | 旧业务字段；实际凭据由 auth_identity 的 Argon2id 哈希校验，拒绝旧明文旁路 |
| name | VARCHAR(64) | 姓名 |
...
| 方法 | 路径 | 说明 | 请求 | 响应 | 角色 |
|------|------|------|------|------|------|
| POST | /api/v1/auth/login | 登录（用户名/邮箱+密码+图片验证码） | LoginReq | LoginResp | 公开 |
| POST | /api/v1/auth/refresh | 续期 Token | RefreshReq | RefreshResp | 公开 |
| POST | /api/v1/auth/logout | 登出 | LogoutReq | LogoutResp | 已登录 |
| POST | /api/v1/auth/admin/login | 管理台登录（account+password+captchaId+captchaCode） | AdminLoginReq | LoginResp | 公开 |

续期接口虽然不要求另带 access token，仍必须验证 Refresh Token 的签名、有效期、类型及绑定身份。Refresh Token 保留原登录身份域与客户端；续期只能查询该身份对应的账号，不按相同数字 ID 跨信众与管理账号表猜测。缺少身份信息的旧 Refresh Token 拒绝续期并要求重新登录。后台 access JWT 的 `UserType=admin` 与角色共同决定权限，刷新不能把信众身份提升为管理员。
...
独立大师在大师端注册申请人并提交认证，平台审核通过后开通业务；寺院经办人在寺院平台注册入驻。纳管大师由本寺建立认证档案、分配用户名与邮箱，本人在大师端邮箱激活并设置密码。两类大师使用同一大师入口，归属及管理权限不同。寺院只能停用 / 恢复本寺纳管账号；申请人不能绕过审核访问业务。当前总平台使用内部 admin，无公众注册入口。
...
2. **当前凭据**：auth_identity 保存分域身份、用户名、邮箱验证时间和 Argon2id 密码哈希；不接受旧固定验证码或默认明文密码。图片验证码一次消费，邮件码 6 位、5 分钟、最多 5 次尝试，Redis 保存 HMAC 并限流。
3. **邮箱与恢复**：生产 SMTP 已部署；注册、工作身份激活、找回与存量绑定使用邮件验证。短信未启用时隐藏；占位测试邮箱仅通过用户名登录，不可收信或找回。
4. **角色绑定**：创建账号时绑定 role_id，寺院管理员须填 temple_id，法师账号须填 master_id
5. **数据隔离**：temple_admin 仅本寺院数据，master 仅本人数据，platform_super 全平台
6. **会话**：JWT sid 关联服务端会话；退出撤销当前会话与刷新能力，改密撤销该身份全部会话，网关校验 sid 与角色
...
| 依赖类型 | 依赖 | 说明 |
|---------|------|------|
| Redis | 验证码 HMAC、限流与会话 | 一次消费、过期、失败次数、sid 撤销与网关校验 |
