...
- **OpenIM 集成**（best-effort）：C 端用户登录时同步注册到 OpenIM 并获取 IMToken，随登录响应返回。OpenIM 不可用时 IMToken 为空字符串，不影响核心业务闭环
...
7. **OpenIM best-effort 集成**：C 端用户登录成功后，若 `IMClient` 非空则调用 `RegisterUser` + `GetUserToken` 获取 IMToken 并写入 `LoginResp.IMToken`；任何 OpenIM 调用失败均忽略，IMToken 保持空字符串，不影响登录主流程
