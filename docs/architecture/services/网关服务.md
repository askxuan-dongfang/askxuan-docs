...
- **反向代理**：按 URL 前缀最长匹配，将请求转发到对应下游微服务（20 条 C 端路由 + 19 条管理台路由）
- **JWT 鉴权**：全局中间件校验 Access Token，白名单路径直接放行，`/api/v1/admin/*` 路径额外校验管理台角色
...
> MVP-1 阶段使用静态路由表（yaml 配置），etcd 服务发现预留但未启用动态路由。
...
| /api/v1/user | localhost:8082 | 用户服务（注册/资料/地址） |
| /api/v1/temples | localhost:8083 | 寺院服务 |
| /api/v1/masters | localhost:8084 | 法师服务 |
| /api/v1/booking | localhost:8085 | 预约服务 |
| /api/v1/message | localhost:8094 | 消息服务（单数前缀） |
| /api/v1/messages | localhost:8094 | 消息服务（复数前缀） |
| /api/v1/announcements | localhost:8094 | 公告服务 |
| /api/v1/file | localhost:8097 | 文件服务 |
...
### 3.2 管理台路由（19 条，前缀 /api/v1/admin/）
...
以下路径不需要 JWT Token，直接放行：

| 路径 | 说明 |
|------|------|
| /api/v1/auth/login | C 端登录 |
| /api/v1/auth/refresh | Token 续期 |
| /api/v1/auth/admin/login | 管理台登录 |
| /api/v1/user/register | 用户注册 |
| /api/v1/payments/callback/wechat | 微信支付回调 |
| /api/v1/payments/callback/alipay | 支付宝支付回调 |
...
2. **管理台角色校验**：`/api/v1/admin/*` 路径除 JWT 校验外，额外检查 `claims.IsAdmin()`（temple_admin/shop_admin/platform_super/platform_service），不满足返回 `ErrRoleForbidden`
...
6. **etcd 预留**：配置中包含 Telemetry.Endpoint 指向 etcd，但 MVP-1 使用静态路由表，未启用动态服务发现
...
| 依赖类型 | 依赖 | 说明 |
|---------|------|------|
| etcd | localhost:2379 | 服务发现（MVP-1 预留，未启用动态路由） |
| Redis | localhost:6379 | Token 黑名单校验（由 auth-service 写入，gateway 读取） |
| 下游服务 | 18 个微服务 + OpenIM | 反向代理转发目标 |
...
| 403 | 40304 | 角色权限不足（ErrRoleForbidden，管理台路径非管理角色） |
