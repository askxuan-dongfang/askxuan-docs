...
| /api/v1/admin/messages | localhost:8094 | 平台超管消息管理；`/master` 子路径使用更具体的法师本人消息权限 |
| /api/v1/admin/announcements | localhost:8094 | 平台超管公告管理 |
...
JWT 校验通过后，网关将用户信息注入以下请求头。平台消息/公告及法师消息的服务自身守卫重新验证 JWT，并从验证后的 claims/context 读取身份；不能由调用者伪造 `X-User-*` 头取得权限：
...
2. **管理台/工作台角色校验**：`/api/v1/admin/*` 的人类调用要求签名 claims 中 `UserType=admin`，再按业务前缀匹配角色；缺少身份的旧 token 不因携带管理角色而放行。平台消息及公告要求 `platform_super`；更具体的 `/api/v1/admin/messages/master` 及子路径要求 `master` 与有效 `MasterID`。只有明确保留的内部集成才接受 `UserType=service` 且 `platform_service`，平台消息/公告不在该放行范围，人类 `platform_service` 角色不等于服务身份
