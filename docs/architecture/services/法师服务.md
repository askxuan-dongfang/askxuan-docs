...
| `GET/PUT /api/v1/admin/masters/service-tags` | 法师端服务标签（S001-S013 目录校验） |
| `POST /api/v1/admin/platform/masters` | 平台创建野生大师（W 编码，初始待审核） |
| 详情/资料响应新增 `manageBy`、`serviceTags` 字段 | 两类大师区分与标签展示 |

### 模型变化
- `master.manage_by`（temple/platform）；野生大师 `temple_code=''`
- 新表 `master_service_tag(master_code, service_code, price, status)`
