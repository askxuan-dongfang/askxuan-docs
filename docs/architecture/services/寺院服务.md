...
> **服务端口**: 8083
> **业务域**: 核心业务域
> **关联文档**: [技术架构](../技术架构.md) | [API规范](../API规范.md) | [状态机](../状态机.md) | [业务流程](../业务流程.md) | [法师与后台操作](../../guides/manual/法师与后台操作.md) | [当前能力与限制](../../product/产品现状与能力边界.md)

---

## 1. 服务职责

temple-service 负责寺院信息、图片、标准服务开通与定价、入驻审核及 **加持任务接收**：
...
| 字段 | 类型 | 说明 |
|------|------|------|
| id | VARCHAR(16) | 主键（寺院编码，当前演示目录 T001~T010） |
| name | VARCHAR(64) | 名称 |
...
### 3.5 temple_service（寺院标准服务开通与定价表）

| 字段 | 类型 | 说明 |
|------|------|------|
| id | BIGINT AUTO_INCREMENT | 主键 |
| temple_code | VARCHAR(16) | 寺院编码 |
| service_code | VARCHAR(32) | 标准服务编码 S001~S013，引用 `service_type.code` |
| service_name | VARCHAR(64) | 标准服务名称，由服务端按编码写入 |
| price | DECIMAL(10,2) | 价格 |
| time_slots | JSON | 兼容旧客户端的时段字符串数组 |
| temple_service_slot | 结构化表 | slot_code/名称/起止时间/容量/启停/排序，预约权威来源 |
...
| GET | /api/v1/service-types | 固定标准服务类型列表 | - | ServiceTypeListResp | 公开 |
...
| GET | /api/v1/admin/temples/images | 本寺院图册 | - | TempleImageListResp | temple_admin |
...
寺院服务类型来自 `service_type` 固定目录。创建时客户端只提交 `serviceCode`，服务端校验编码并写入标准名称；同一寺院不能重复开通同一编码。寺院端不提供自定义编码或名称输入，只维护价格、时段容量、上下架和诉求标签。

寺院封面与图册由寺院管理台上传到 file-service，C 端寺院详情只读取 `coverImage` 与 `images[].url`。平台总管理台负责查看与审核，不维护各寺院日常图册。

公开详情固定返回 `TempleDetail { temple, images, services }`，其中 `services` 仅含已上架项目，寺院摘要中的 `serviceCodes/serviceTags/serviceCount` 从同一批服务实时计算。初始化及前向迁移保证每座寺院至少有一条从 `coverImage` 同步的封面图记录；待审核、封禁寺院仍只可由平台查看。

### 4.3 平台管理台接口（需JWT + 平台超管）

| 方法 | 路径 | 说明 | 请求 | 响应 | 角色 |
|------|------|------|------|------|------|
| GET | /api/v1/admin/platform/temples | 全部寺院及上架服务摘要 | ListReq | ListResp | platform_super |
| GET | /api/v1/admin/platform/temples/:id | 全状态寺院详情 | DetailReq | TempleDetail | platform_super |
| GET | /api/v1/admin/platform/temples/audits | 入驻审核列表，可按 templeCode 定位 | TempleAuditListReq | TempleAuditListResp | platform_super |
...
| PUT | /api/v1/admin/platform/temples/:id/status | 正常/封禁/推荐（待审核不可绕过审核） | TemplePlatformStatusReq | TemplePlatformStatusResp | platform_super |
...
5. **服务管理**：寺院自主定价并维护结构化时段和容量；`time_slots` 仅作为兼容输出
6. **预约 gRPC**：`temple.rpc`（9083）向 booking-service 返回上架状态、当前价格和结构化时段
