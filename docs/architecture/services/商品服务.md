...
- C端商品浏览（列表/详情/分类树）
- 承载平台心愿导航主数据
- 跨库只读聚合寺院服务与大师服务，不复制服务、预约或咨询订单数据
...
### 3.5 intent_tag 与服务映射

`intent_tag` 是平台运营维护的心愿导航主数据，客户端不写死名称、数量和排序。业务可办理项来自固定 13 项 `service_type`：寺院服务关联保存在 `askxuan_temple.temple_service_intent_tag`，大师服务保存在 `askxuan_master.master_service_tag`。`product_intent_tag` 仅为旧版数据兼容保留，当前代码不读写，商品不进入“按心愿办”。DIY 仍是独立直达业务，不参与聚合列表。
...
| GET | /api/v1/intentions/tags | 启用的心愿分类 | - | IntentionTagListResp | customer |
| GET | /api/v1/intentions | 寺院服务与大师服务聚合 | code, page, size | CustomerIntentionResp | customer |
...
平台管理台使用 `/api/v1/admin/platform/intentions` 创建、编辑、排序和启停心愿导航主数据。寺院管理台从固定服务目录开通服务并维护心愿关联；大师服务标签由平台/寺院按大师归属权限维护。商城管理台不参与心愿配置。
...
9. **诉求主数据**：平台管理台维护入口；C 端通过 `/intentions/tags` 动态读取启用项，不保留固定名称或固定数量。
10. **心愿聚合**：寺院必须正常且服务上架；大师必须已认证、正常且服务已启用。Product Service 使用全限定表名只读 `askxuan_temple` 与 `askxuan_master`，返回预约/大师服务落地目标，不复制订单逻辑。
...
| MySQL | askxuan_product 库 | product/product_sku/product_category/product_image/intent_tag 表；product_intent_tag 仅历史兼容 |
| MySQL（跨库只读） | askxuan_temple 库 | temple/temple_service/temple_service_intent_tag 表 |
| MySQL（跨库只读） | askxuan_master 库 | master/master_service_tag 表 |
| Redis | 本地 | 商品列表/详情缓存 |
| etcd | 服务注册 | 服务发现 |
...
| v1.1 | 2026-07-13 | 增加诉求标签、跨域混合聚合与管理台标签维护 |
| v1.2 | 2026-08-27 | 心愿聚合收口为寺院服务 + 大师服务，商品与 DIY 保持独立入口 |
