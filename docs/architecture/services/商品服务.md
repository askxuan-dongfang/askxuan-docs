...
### 3.5 intent_tag / product_intent_tag

`intent_tag` 是平台运营维护的动态诉求主数据，默认种子为 `peace`、`wealth`、`love`、`career`、`study`、`taisui`、`diy`、`rite`，但客户端不得据此写死。记录同时控制图标、排序、启停和 `aggregate/service/diy` 落地方式。`product_intent_tag` 仅保存商品映射；寺院服务映射由 `askxuan_temple.temple_service_intent_tag` 持有。
...
| GET | /api/v1/intentions | 商品与寺院服务混合聚合 | code, page, size | CustomerIntentionResp | customer |
...
平台管理台使用 `/api/v1/admin/platform/intentions` 创建、编辑、排序和启停心愿主数据；商城管理台只负责将商品关联到当前标签，不能另建一套分类。
...
10. **诉求聚合**：商品必须上架且有库存；寺院必须正常且服务上架。Product Service 使用全限定表名只读 `askxuan_temple`，返回下单目标但不复制订单逻辑。
...
| MySQL | askxuan_product 库 | product/product_sku/product_category/product_image/intent_tag/product_intent_tag 表 |
