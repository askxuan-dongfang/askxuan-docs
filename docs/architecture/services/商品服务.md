...
`intent_tag` 维护八个稳定诉求编码：`peace`、`wealth`、`love`、`career`、`study`、`taisui`、`diy`、`rite`。`product_intent_tag` 仅保存商品映射；寺院服务映射由 `askxuan_temple.temple_service_intent_tag` 持有。
...
9. **诉求聚合**：商品必须上架且有库存；寺院必须正常且服务上架。Product Service 使用全限定表名只读 `askxuan_temple`，返回下单目标但不复制订单逻辑。
