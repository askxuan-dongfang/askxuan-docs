...
- **后端代传**：管理台图片和小文件通过 `POST /upload` 走后端 `multipart/form-data` 上传，最大 32MB
- **公网对象地址**：MinIO `Endpoint` 用于容器内连接，`PublicBaseURL` 用于生成浏览器和 iPhone 可访问的 HTTPS URL
...
```json
{
  ...
  "url": "https://101.96.228.71/objects/askxuan/temp/20260701000000_avatar.png",
  "size": 102400,
...
10. **管理边界**：寺院图片由寺院台维护；流派/Banner 由平台台维护；商品/DIY 图片由商城台维护；各客户端只消费业务 API 返回的 URL
