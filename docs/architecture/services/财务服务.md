...
graph TB
    ...
    subgraph 数据层
        MYSQL[("MySQL<br/>settlement/withdrawal<br/>commission_config/finance_log")]
        REDIS[("Redis<br/>缓存/锁")]
    end

    subgraph 外部依赖
        BOOKING["booking-service<br/>RPC查询预约订单"]
        ORDER["order-service<br/>RPC查询商城订单"]
...
1. 订单完成（预约reviewed / 商城completed）后，finance-service 消费 MQ 事件
2. 按月汇总各对象（寺院/法师/商城）的收入
3. 根据业务类型读取 commission_config 获取抽成比例
4. 计算平台抽成金额与实结金额，生成 settlement 记录
5. 平台超管确认结算（status: pending → confirmed）
6. 提现打款完成后更新结算单为 paid
...
4. 打款结果回调（processing → success / failed）
...
| 依赖类型 | 依赖服务 | 用途 |
|---------|---------|------|
| RPC | booking-service | 查询预约订单完成状态与金额 |
| RPC | order-service | 查询商城/DIY订单完成状态与金额 |
